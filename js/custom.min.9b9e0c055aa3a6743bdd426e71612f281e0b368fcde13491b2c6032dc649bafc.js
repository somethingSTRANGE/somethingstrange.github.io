var bufObj={},ShProg={},frameCount=0,canvas,textureObj,maskTextureObj,gl,prog,myGif,inputBrightness,inputCurvature,inputScanlineOpacity,inputVignetteOpacity,inputVignetteRoundness,inputDistortion,inputColorShift,inputStripes,VertexBuffer,Texture;const gifURL="/image/crt/cthulhu.gif";setTimeout(()=>{myGif=GIF(),myGif.onerror=function(a){console.log("Gif loading error "+a.type)},myGif.load(gifURL)},0);function input(a,b){return a?a.value:b}function render(l){let a,b;const c=0;gl.viewport(0,0,canvas.width,canvas.height),gl.enable(gl.DEPTH_TEST),gl.clearColor(0,0,0,1),gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT),gl.activeTexture(gl.TEXTURE0+c),myGif?myGif.loading?myGif.lastFrame!==null&&(a=myGif.lastFrame.image,b=gl.createTexture(),gl.bindTexture(gl.TEXTURE_2D,b),gl.texImage2D(gl.TEXTURE_2D,0,gl.RGBA,gl.RGBA,gl.UNSIGNED_BYTE,a),gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_WRAP_S,gl.CLAMP_TO_EDGE),gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_WRAP_T,gl.CLAMP_TO_EDGE),gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MIN_FILTER,gl.LINEAR)):(a=myGif.image,b=gl.createTexture(),gl.bindTexture(gl.TEXTURE_2D,b),gl.texImage2D(gl.TEXTURE_2D,0,gl.RGBA,gl.RGBA,gl.UNSIGNED_BYTE,a),gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_WRAP_S,gl.CLAMP_TO_EDGE),gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_WRAP_T,gl.CLAMP_TO_EDGE),gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MIN_FILTER,gl.LINEAR)):ctx.fillText("Waiting for GIF image ",4,11);let f=input(inputBrightness,1.2),d=input(inputCurvature,3.25),e=input(inputScanlineOpacity,.25),g=input(inputVignetteOpacity,1),h=input(inputVignetteRoundness,1),i=input(inputDistortion,20)/100,j=input(inputColorShift,0)/1e3,k=input(inputStripes,0)/100;ShProg.Use(progDraw),ShProg.SetF2(progDraw,"screenResolution",[180,135]),ShProg.SetI1(progDraw,"u_texture",c),ShProg.SetF1(progDraw,"brightness",f),ShProg.SetF2(progDraw,"u_curvature",[d,d]),ShProg.SetF2(progDraw,"scanLineOpacity",[0,e]),ShProg.SetF1(progDraw,"vignetteOpacity",g),ShProg.SetF1(progDraw,"vignetteRoundness",h),ShProg.SetF1(progDraw,"u_distortion",i),ShProg.SetF1(progDraw,"u_rgbshift",j),ShProg.SetF1(progDraw,"u_stripe",k),VertexBuffer.Draw(bufRect),requestAnimationFrame(render)}function vertexShader(){return`
